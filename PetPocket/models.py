...
class User(UserMixin, db.Model):
    ...
    page_views = db.relationship('PageView', backref='user', lazy=True, cascade='all, delete-orphan')
    product_views = db.relationship('ProductView', backref='user', lazy=True, cascade='all, delete-orphan')
...
class PetType(db.Model):
    ...
    products = db.relationship('Product', backref='pet_type', lazy=True, cascade='all, delete-orphan')
...
class Product(db.Model):
    ...
    discount_price = db.Column(db.Float, nullable=True)  # NEW FIELD
    stock = db.Column(db.Integer, default=0)
    image_url = db.Column(db.String(200))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    weight = db.Column(db.String(50), nullable=True)  # CHANGED from Float to String
    ...
    # Define one-to-many relationship with ProductAttribute
...
class ProductAttribute(db.Model):
    ...
    # Backref is defined on the Product side
...
class SalesAnalytics(db.Model):
    ...
    def update_daily_stats(cls, date):
        ...
        if record:
            ...
            record = cls(date=date, total_sales=total_sales, order_count=order_count, avg_order_value=avg_order_value)
