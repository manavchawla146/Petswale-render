document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sort functionality - updated to handle discount prices
            ...
            try {
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        setTimeout(() => window.location.href = '/signin?notify=cart', 1500);
                        return;
                    }
                    throw new Error(`HTTP ${response.status}: ${response.statusText}`);
                }
            ...
            try {
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        setTimeout(() => window.location.href = '/signin?notify=wishlist', 1500);
                        return;
                    }
                    throw new Error(`HTTP ${response.status}: ${response.statusText}`);
                }
